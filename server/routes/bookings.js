...
router.get('/', async (req, res) => {
  try {
    ...
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});

// Booking stats and SLA - MUST come BEFORE /:id to avoid matching "stats" as an ID
router.get('/stats', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('GET /bookings/stats error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});

// Get detailed booking with joined items/tests - MUST come AFTER /stats route
router.get('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('GET /bookings/:id error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});
...
router.put('/:id/status', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('PUT /bookings/:id/status error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});
...
router.put('/:id/cancel', requireAuth, async (req, res) => {
  try {
    ...
    console.error('PUT /bookings/:id/cancel error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});
...
router.get('/:id/notes', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('GET /bookings/:id/notes error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});
...
router.post('/:id/notes', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('POST /bookings/:id/notes error:', e.message);
    return res.status(503).json({ message: 'Database error', error: e.message });
  }
});
